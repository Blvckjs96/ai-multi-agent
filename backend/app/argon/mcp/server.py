"""
Argon MCP Server — exposes Knowledge Base tools to Claude.
...
Architecture:
    Claude Desktop → MCP (HTTPS) → /mcp endpoint → Argon KB tools
                                                   → PostgreSQL (pgvector)
                                                   → Neo4j (graph)
                                                   → MinIO (files)

Connection:
    Employee runs: argon connect --server https://ai.company.internal --token <token>
...
from app.argon.mcp.middleware import ScopedToolsMiddleware
from app.argon.mcp.resources import register_resources
from app.argon.mcp.tools import register_tools


def create_mcp_server() -> FastMCP:
    """
    Create and configure the Argon MCP server.
    Call this once during app startup.
    """
    mcp = FastMCP(
        "Argon",
        instructions=(
            "You are connected to Argon — the company's internal knowledge base. "
            "\n\n"
            "## MANDATORY: Always search Argon before answering\n"
            ...
            "you MUST query Argon first. Do NOT answer from general knowledge alone "
    ...
    # registered so the middleware can read `__argon_requires__` off each fn.
