"""
Argon MCP Tools — LLM Wiki + raw source drill-down for Claude.
...
from app.argon.mcp.logging import current_identity, logged_tool
from app.argon.mcp.permissions import (
    ANY_AUTHENTICATED,
    CAN_CONTRIBUTE_WIKI,
...
async def _get_identity():
    ...
    from app.argon.services.mcp_auth_service import MCPAuthService
...
async def _get_allowed_source_ids(identity, session: Optional[AsyncSession] = None) -> Optional[set[str]]:
    ...
    from app.argon.services.mcp_auth_service import apply_scope_filter
...
async def _can_review_page(session: AsyncSession, employee, page) -> bool:
    ...
    from app.argon.services.permission_engine import (
...
async def _format_oos_hint(session: AsyncSession, oos_hits: list) -> str:
    ...
    from app.argon.db_compat_models import Department, Project
...
def register_tools(mcp: FastMCP):
    ...
    async def search_wiki(query: str, top_k: int = 10) -> str:
        ...
        from app.argon.db_compat import async_session_factory
        from app.argon.services import wiki_service
    ...
    async def read_wiki_page(slug: str) -> str:
        ...
        from app.argon.db_compat_models import Department, Project, WikiPage
    ...
    async def get_source_pages(source_id: str, pages: str) -> str:
        ...
        from app.argon.db_compat import async_session_factory
        from app.argon.db_compat_models import Source
        from app.argon.services.source_outline import parse_page_range, slice_pages_by_range
    ...
    async def propose_wiki_edit(
        ...
        from app.argon.db_compat import async_session_factory
        from app.argon.db_compat_models import Employee, WikiPage
        from app.argon.services import wiki_service
        ...
        async with async_session_factory() as session:
            ...
            draft.page = page
            from app.argon.services import contribution_service
            from app.argon.services.contribution_service import wiki_draft_adapter
    ...
    async def list_pending_drafts(
        ...
        from app.argon.db_compat import async_session_factory
        from app.argon.db_compat_models import (
            Employee,
            ProjectMember,
            WikiPage,
            WikiPageDraft,
        ...
        from app.argon.services.permission_engine import _get_user_permissions
    ...
    async def review_draft(draft_id: str) -> str:
        ...
        from app.argon.db_compat import async_session_factory
        from app.argon.db_compat_models import Employee, WikiPageDraft
    ...
    async def request_changes_on_draft(draft_id: str, reviewer_note: str) -> str:
        ...
        from app.argon.db_compat_models import Employee, WikiPageDraft
        from app.argon.services import contribution_service
        from app.argon.services.contribution_service import (
            InvalidTransition,
            wiki_draft_adapter,
        )
    ...
    async def resubmit_draft(
        ...
        from app.argon.db_compat_models import Employee, WikiPageDraft
        from app.argon.services import contribution_service
        from app.argon.services.contribution_service import InvalidTransition
    ...
    async def propose_wiki_create(
        ...
        from app.argon.db_compat import async_session_factory
        from app.argon.db_compat_models import Employee
        from app.argon.services import contribution_service, wiki_service
        from app.argon.services.contribution_service import wiki_draft_adapter
