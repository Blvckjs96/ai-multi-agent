...
Configuration keys (read via ConfigService):
  ...
  smtp_from              -> string e.g. "Argon <noreply@argon.example>"
  smtp_use_tls           -> "true" / "false" (default "true")

  webhook_enabled        -> "true" / "false"
  webhook_url            -> string (single endpoint; multi-URL is a future ext)
  webhook_secret         -> optional HMAC secret (sent in X-Argon-Signature)
...
from app.argon.db_compat_models import Employee, Notification
...
async def _dispatch_email_batch(
    ...
    try:
        ...
        from_addr = await cfg.get("smtp_from") or "argon@localhost"
    ...
    for recipient_id, items in by_recipient.items():
        ...
        subject = (
            ...
            else f"Argon — {len(items)} new notifications"
        )
...
def _build_email_body(name: str, items: list[Notification]) -> str:
    ...
    if len(items) == 1:
        ...
        lines.append("You have new activity on Argon:")
        lines.append("")
        for n in items:
        ...
        lines.append("")
    lines.append("Open Argon to review.")
    return "\n".join(lines)
...
async def _dispatch_webhook_batch(
    ...
    headers = {"Content-Type": "application/json", "User-Agent": "Argon-Webhook/1"}
    if secret:
        sig = hmac.new(secret.encode(), body_bytes, hashlib.sha256).hexdigest()
        headers["X-Argon-Signature"] = f"sha256={sig}"
