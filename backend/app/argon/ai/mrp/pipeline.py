...
from app.argon.ai.mrp.mapper import run_map_phase
from app.argon.ai.mrp.reducer import run_reduce_phase
from app.argon.ai.mrp.verifier import run_verify_phase
from app.argon.ai.mrp.writer import PageWriteResult, run_refine_phase
from app.argon.utils.progress import ProgressTracker
...
async def _resolve_wiki_scopes(session: AsyncSession, source) -> list[tuple[str, Optional[uuid.UUID]]]:
    ...
    """
    from app.argon.db_compat_models import SourceDepartment
...
async def run_commit_phase(
    ...
    """
    from app.argon.ai.mrp.merger import merge_page_content
...
async def _load_plan(session: AsyncSession, source_id: uuid.UUID):
    ...
    from app.argon.db_compat_models import SourceCompilationPlan
...
async def _load_chunk_extracts(session: AsyncSession, source_id: uuid.UUID) -> list:
    ...
    from app.argon.db_compat_models import SourceChunkExtract
    rows = (await session.execute(
...
async def _auto_trigger_refine(source_id: uuid.UUID, plan) -> dict:
    ...
    from app.argon.worker import get_arq_pool
