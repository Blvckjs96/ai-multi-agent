...
from app.argon.ai.providers.base import (
    EmbeddingProvider,
    LLMProvider,
    ProviderConfig,
    ProviderType,
...
def _get_embedding_class(provider: ProviderType) -> type[EmbeddingProvider]:
    if provider == ProviderType.GOOGLE:
        from app.argon.ai.providers.google import GoogleEmbedding
        return GoogleEmbedding
    elif provider == ProviderType.OPENAI:
        from app.argon.ai.providers.openai_provider import OpenAIEmbedding
...
def _get_llm_class(provider: ProviderType) -> type[LLMProvider]:
    if provider == ProviderType.GOOGLE:
        from app.argon.ai.providers.google import GoogleLLM
        return GoogleLLM
    elif provider == ProviderType.OPENAI:
        from app.argon.ai.providers.openai_provider import OpenAILLM
        return OpenAILLM
    elif provider == ProviderType.ANTHROPIC:
        from app.argon.ai.providers.anthropic_provider import AnthropicLLM
        return AnthropicLLM
...
def _get_vision_class(provider: ProviderType) -> type[VisionProvider]:
    if provider == ProviderType.GOOGLE:
        from app.argon.ai.providers.google import GoogleVision
        return GoogleVision
    elif provider == ProviderType.OPENAI:
        from app.argon.ai.providers.openai_provider import OpenAIVision
        return OpenAIVision
...
class ProviderRegistry:
    ...
    async def get_active_embedding_spec_id(self) -> Optional[str]:
        ...
        from app.argon.ai.embedding_catalog import EMBEDDING_CATALOG
    ...
    async def get_active_llm_spec_id(self) -> Optional[str]:
        ...
        from app.argon.ai.llm_catalog import LLM_CATALOG, derive_spec_id
        from app.argon.services.config_service import ACTIVE_LLM_MODEL_KEY, ConfigService
    ...
    async def get_active_vision_spec_id(self) -> Optional[str]:
        ...
        from app.argon.ai.vision_catalog import VISION_CATALOG, derive_spec_id
        from app.argon.services.config_service import (
    ...
    async def _load_llm_config(self) -> ProviderConfig:
        ...
        """
        from app.argon.ai.llm_catalog import get_spec
        from app.argon.services.config_service import ConfigService
    ...
    async def _load_vision_config(self) -> ProviderConfig:
        ...
        from app.argon.ai.vision_catalog import get_spec
        from app.argon.services.config_service import ConfigService
...
def supported_providers() -> dict:
    ...
    """
    from app.argon.ai.embedding_catalog import EMBEDDING_CATALOG
    from app.argon.ai.llm_catalog import LLM_CATALOG
    from app.argon.ai.vision_catalog import VISION_CATALOG
