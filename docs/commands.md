...
```bash
cd backend
uv run ai_multi_agent <group> <command> [options]
```

### Server Commands

```bash
uv run ai_multi_agent server run              # Start dev server
uv run ai_multi_agent server run --reload     # With hot reload
uv run ai_multi_agent server run --port 9000  # Custom port
uv run ai_multi_agent server routes           # Show all registered routes
```

### Database Commands

```bash
uv run ai_multi_agent db init                  # Run all migrations
uv run ai_multi_agent db migrate -m "message"  # Create new migration
uv run ai_multi_agent db upgrade               # Apply pending migrations
uv run ai_multi_agent db upgrade --revision e3f  # Upgrade to specific revision
uv run ai_multi_agent db downgrade             # Rollback last migration
uv run ai_multi_agent db downgrade --revision base  # Rollback to start
uv run ai_multi_agent db current               # Show current revision
uv run ai_multi_agent db history               # Show migration history
```
...
uv run ai_multi_agent user create

# Create user non-interactively
uv run ai_multi_agent user create --email user@example.com --password secret

# Create user with specific role
uv run ai_multi_agent user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run ai_multi_agent user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run ai_multi_agent user create-admin --email admin@example.com --password secret

# Change user role
uv run ai_multi_agent user set-role user@example.com --role admin

# List all users
uv run ai_multi_agent user list
```

### Celery Commands

```bash
uv run ai_multi_agent celery worker                    # Start worker
uv run ai_multi_agent celery worker --loglevel debug   # Debug logging
uv run ai_multi_agent celery worker --concurrency 8    # 8 worker processes
uv run ai_multi_agent celery beat                      # Start scheduler
uv run ai_multi_agent celery flower                    # Start Flower UI
uv run ai_multi_agent celery flower --port 5556        # Custom Flower port
```
...
```bash
uv run ai_multi_agent cmd <command-name> [options]
```
...
```bash
uv run ai_multi_agent cmd my-command --name test
```
